...
class FavouriteRepository {
  ...
  Future<FavouriteModel> addToFavourite(
      {required String userId, required String routeId}) async {
    try {
      final data = {'user_id': userId, 'route_id': routeId};
      final response =
          await _supabase.from('favourites').insert(data).select().single();
  ...
  Future<void> removeFromFavourites(
    ...
    try {
      await _supabase
          .from('favourites')
          .delete()
          .match({'user_id': userId, 'route_id': routeId});
    } catch (e) {
      print(e.toString());
    ...
    try {
      ...
      return response.map((item) {
        ...
        return RouteModel(
          ...
          photoUrls: (route['photo_urls'] as List<dynamic>?)
                  ?.map((e) => e as String)
                  .toList() ??
              [],
          userId: route['user_id'] as String,
          ...
          creatorRating: route['creator_rating'] as double?,
          creatorRoutesCount: route['creator_routes_count'] as int?,
        );
      }).toList();
    } catch (e) {
      print(e.toString());
  ...
  Future<bool> isRouteFavourite(
      {required String routeId, required String userId}) async {
    try {
      final response = await _supabase
          .from('favourites')
          .select()
          .match({'user_id': userId, 'route_id': routeId}).maybeSingle();
